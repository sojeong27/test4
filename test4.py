...
if question == '지식':
    quset = '1. 책에 어떤 동물이 나왔나요?', '2. 이 동물의 이름은 무엇인가요?', '3. 달팽이는 어떤 음식을 좋아하나요?'
if question == '이해':
    quset = '1. 달팽이는 동물일까요, 곤충일까요?', '2. 달팽이처럼 느린 동물은 또 무엇이 있을까요?', '3. 달팽이를 키울 때 주의해야 할 점은 무엇인가요?'
if question == '적용':
    quset = '1. 거북이 모양으로 만든 물건에는 어떤 것들이 있나요?', '2. 달팽이와 거북이는 어떤 점이 비슷한가요? ', '3. 만약 나에게도 달팽이처럼 등껍질이 있다면 어떻게 사용할 수 있을까요?'

if keyword and question:
    preset_text = f'키워드를 포함하여 질문 유형과 동일한 의미의 다른 표현 5개를 생성합니다.\n\n키워드:{keyword}\n\n질문 유형:{quset}'
