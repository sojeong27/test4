...
if question == '지식':
    quset = '생김새', '이름', '색깔', '좋아하는 것', '뜻', '의미', '모양'
if question == '이해':
    quset = '같은 종류는', '또 다른 예는', '비슷한 것은', '다른 것', '같은 것', '비교', '분류'
if question == '적용':
    quset = '비슷한 느낌의 물건', '만약 나라면', '모양이 변한다면', '만약 없다면'

if keyword and question:
    preset_text = f'키워드와 질문 유형을 포함하여 질문만 생성합니다. 각 문장마다 번호를 붙여 질문만 10개를 제시합니다. 문장이 끝나면 엔터를 칩니다. 답변 또는 답변 예시는 생성하지 않습니다.\n\n키워드:{keyword}\n\n질문 유형:{quset}'
